 ...
 */
export class AclSeed extends BaseSeeder<AclContext> {
  async run(dataSource: DataSource, context: Context = {}): Promise<void> {
    ...
    await this.call(dataSource, [UsersSeed, CategorySeed], aclContext);
    await this.call(dataSource, [TagSeed], aclContext);
    await this.call(dataSource, [PostSeed], aclContext);
    await this.call(dataSource, [CommentSeed, DocumentSeed, ArticleSeed], aclContext);
