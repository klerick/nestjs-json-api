...
describe('AJV factory', () => {
  ...
  it('Should be have schema for input', () => {
    ...
    const inputQuerySchema = ajvCall.getSchema<Users>(`inputQuerySchema-${Users.name}`);
    const repository = dataSource.getRepository<Users>(Users);
    const relationField = repository.metadata.relations.reduce((acum, item) => {
      const relations = item.inverseEntityMetadata.relations.map(i => i.propertyName);
      acum.push(
    ...
    expect(inputQuerySchema.schema).toHaveProperty('properties.fields.properties.target');
    ...
    for (const propertyName of relationField) {
      if (propertyName === 'manager.lastName') {
        expect(inputQuerySchema.schema['properties'].filter.properties).not.toHaveProperty(
          [propertyName]
        );
      } else {
        expect(inputQuerySchema.schema['properties'].filter.properties).toHaveProperty(
          [propertyName]
        );
      }
    }
  ...
  it('Should be have schema for transform', () => {
    const transformQuerySchema = ajvCall.getSchema<QueryParams<Users>>(`transformQuerySchema-${Users.name}`);
    const repository = dataSource.getRepository<Users>(Users);
    const relationField = repository.metadata.relations.reduce((acum, item) => {
      const relations = item.inverseEntityMetadata.relations.map(i => i.propertyName);
      acum.push(
    ...
    const propsColumns = repository.metadata.columns.filter(i => {
    ...
    for (const propertyName in relationObject) {
      expect(relationObject[propertyName]).toEqual(
        transformQuerySchema.schema['$defs'].fieldsDefs.properties[propertyName].items.enum
      );
    }

    expect(relationProps).toEqual(transformQuerySchema.schema['$defs'].includeDefs.items.enum);
    expect(targetProps).toEqual(transformQuerySchema.schema['$defs'].fieldsDefs.properties.target.items.enum)
  })

  it('Should be have schema for input body', () => {
    const transformQuerySchema = ajvCall.getSchema<QueryParams<Users>>(`inputBodyPostSchema-${Users.name}`);
    ...
    const propsColumns = repository.metadata.columns.filter(i => {
      return !relationField.includes(i.propertyName) && i.propertyName !== 'id'
    }).map(i => i.propertyName).filter(i => i !== 'lastName')
    expect(transformQuerySchema.schema).toHaveProperty(
      'properties'
    );
    expect(transformQuerySchema.schema).toHaveProperty(
    ...
    expect(transformQuerySchema.schema['properties']).toHaveProperty(
      'data'
    );
    expect(transformQuerySchema.schema['properties']['data']).toHaveProperty(
      'required'
    );
    expect(transformQuerySchema.schema['properties']['data']['required']).toEqual(['type', 'attributes']);

    const dataProperty = transformQuerySchema.schema['properties']['data']['properties']

    expect(dataProperty['relationships']['properties']['comments']['properties']['data']['type']).toBe('array')
    expect(dataProperty['relationships']['properties']['comments']['properties']['data']['items'])
      .toEqual({
        type: 'object',
        properties: {
          id: {
    ...
    expect(Object.keys(dataProperty['relationships']['properties'])).toEqual(relationField)
    expect(dataProperty).toHaveProperty('attributes');
    expect(Object.keys(dataProperty['attributes']['properties'])).toEqual(propsColumns);
  })
});
