...
export function inputBodyPostSchema(
  ...
  const json: typeof inputBodySchemaJson = JSON.parse(JSON.stringify(inputBodySchemaJson));
  json.$id = `${json.$id}/${schemaName}`
  json.properties.data.properties.type.enum.push(camelToKebab(getEntityName(entity)))
  ...
  const attributes = fieldsArray.filter(item => item !== 'id').reduce((acum, item) => {
  ...
  const relationships = Object.keys(relationsField).reduce((acum, item) => {
    ...
    const resultSchema = {
      ...
      properties: {
        ...
        type: {
          ...
          enum: [camelToKebab(getEntityName(arrayPropsConfig.relationType[item]))]
        }
      }
    }
    ...
    acum[item] = {
      ...
      properties: {
        ...
        data: Reflect.getMetadata('design:type', entity['prototype'], item) === Array ? {
          type: 'array',
          items: resultSchema,
          minItems: 1,
        } : resultSchema
