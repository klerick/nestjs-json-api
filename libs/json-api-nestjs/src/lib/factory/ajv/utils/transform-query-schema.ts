...
export function transformQuerySchema(
  ...
  const json: typeof transformQuerySchemaJson = JSON.parse(JSON.stringify(transformQuerySchemaJson));
  json.$id = `${json.$id}/${schemaName}`

  const fieldSchema = fieldsArray.reduce((acum, item) => {
    acum['sort'][item] = {
      $ref: '#/$defs/sortSchema'
    }
  ...
  const relationSchema = Object.entries(relationsField).reduce((acum, [key, value]) => {
