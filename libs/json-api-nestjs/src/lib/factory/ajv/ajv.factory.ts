...
export function AjvCallFactory(dataSource: DataSource, options: ModuleOptions): AjvCall {
  ...
  for (const entity of options.entities) {
    ...
    const columns = repository.metadata.columns.filter(i => !relations.includes(i.propertyName)).map(i => {
    ...
    const fakeObject = columns.reduce<Record<string, string>>((acum, item) => (acum[item] = '', acum), {});
    const fieldsArray = Object.keys(plainToClass(repository.target as any, fakeObject));

    const relationsField = repository.metadata.relations.reduce<Record<string, string[]>>((acum, item) => {
      ...
      const columns = item.inverseEntityMetadata.columns
        ...
        .map(i => {
          relationArrayProps[item.propertyName] = relationArrayProps[item.propertyName] || {};
          relationArrayProps[item.propertyName][i.propertyName] = i.isArray;
          return i.propertyName
        });
      const fakeObject = columns.reduce<Record<string, string>>((acum, item) => (acum[item] = '', acum), {});

      acum[item.propertyName] = Object
        .keys(plainToClass(item.inverseEntityMetadata.target as any, fakeObject));
      return acum;
    }, {});
    const schemaName = entity instanceof Function ? entity.name : entity.options.name;
    AjvCallInst.addSchema(
      inputQuerySchema(entity, fieldsArray, relationsField, `inputQuerySchema-${schemaName}`), `inputQuerySchema-${schemaName}`
    )
    AjvCallInst.addSchema(
      transformQuerySchema(
        entity,
