...
describe('BodyInputPostPipe', () => {
  ...
  it('should be error, additional property and incorrect type', async () => {
    ...
    try{
      ...
      expect(e.response.message[0].detail).toBe('Must NOT have additional properties. Additional Property is: "id"')
      expect(e.response.message[1].detail).toBe('Must be equal to one of the allowed values. Allowed values are: "users"')
      expect(e.response.message[2].detail).toBe('Must NOT have additional properties. Additional Property is: "test"')
      expect(e.response.message[3].detail).toBe('Must be string')
      expect(e.response.message[4].detail).toBe('Must NOT have additional properties. Additional Property is: "test"')
      expect(e.response.message[5].detail).toBe('Must have required property \'data\'')
      expect(e.response.message[6].detail).toBe('Must have required property \'data\'')
      expect(e).toBeInstanceOf(UnprocessableEntityException);
      expect(e.response.message.length).toBeGreaterThan(0)
    }
  ...
  it('Should be error, relationships array is empty', async () => {
    ...
    try{
      ...
      expect(e.response.message[0].detail).toBe('Must NOT have fewer than 1 items')
      expect(e).toBeInstanceOf(UnprocessableEntityException);
      expect(e.response.message.length).toBeGreaterThan(0)
    }
  });
