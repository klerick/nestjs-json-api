...
function factoryMixin(
  ...
  const pipeClass = nameIt(`${entityName.charAt(0).toUpperCase() + entityName.slice(1)}QuerySchemaPipe`, pipe) as PipeMixin;

  Injectable()(pipeClass);
  InjectRepository(entity, connectionName)(pipeClass, 'repository', 0);

  if ('inject' in pipeClass && pipeClass['inject'] && pipeClass['inject'] instanceof Function) {
...
export function querySchemaMixin(entity: Entity, connectionName: string): PipeMixin {
  return factoryMixin(entity, connectionName, QuerySchemaPipe)
}

export function queryTransformMixin(entity: Entity, connectionName: string): PipeMixin {
  return factoryMixin(entity, connectionName, QueryTransformPipe)
}

export function queryTransformSchemaMixin(entity: Entity, connectionName: string): PipeMixin {
  return factoryMixin(entity, connectionName, QueryTransformSchemaPipe)
}

export function queryFiledInIncludeMixin(entity: Entity, connectionName: string): PipeMixin {
  return factoryMixin(entity, connectionName, QueryFiledInIncludePipe)
}

export function bodyInputPostMixin(entity: Entity, connectionName: string): PipeMixin {
  return factoryMixin(entity, connectionName, BodyInputPostPipe)
}

export function bodyInputPatchPipeMixin(entity: Entity, connectionName: string): PipeMixin {
  return factoryMixin(entity, connectionName, BodyInputPatchPipe)
}

export function parseRelationshipNameMixin(entity: Entity, connectionName: string): PipeMixin {
