...
describe('BodyRelationshipPipe', () => {
  ...
  it('Should be error, incorrect schema', async () => {
    ...
    try{
      ...
      expect(e.response.message[0].detail).toBe("Must have required property 'type'")
      expect(e.response.message[1].detail).toBe("Must have required property 'id'")
      expect(e.response.message[2].detail).toBe("Must be array")
      expect(e.response.message[3].detail).toBe('Must match exactly one schema: "object" or "array"')
  ...
  it('Should be error, incorrect id is no number string', async () => {
    ...
    try{
      ...
      expect(e.response.message[2].detail).toBe('Must match exactly one schema: "object" or "array"')
      expect(e.response.message.length).toBeGreaterThan(0)
    }
  })
  ...
  it('Should be error, array is empty', async () => {
    ...
    try{
      ...
      expect(e.response.message[1].detail).toBe("Must NOT have fewer than 1 items")
      expect(e.response.message[2].detail).toBe('Must match exactly one schema: "object" or "array"')
      expect(e.response.message.length).toBeGreaterThan(0)
    }
  })
  ...
  it('Should be error, array with incorrect items', async () => {
    ...
    try{
      ...
      expect(e.response.message[3].detail).toBe("Must have required property 'type'")
