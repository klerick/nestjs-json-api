...
describe('BodyRelationshipPatchPipe', () => {
  ...
  beforeAll(async () => {
    ...
    pipe = module.get<BodyRelationshipPatchPipe<Users>>(BodyRelationshipPatchPipe);
  });
  ...
  it('Should be error, incorrect schema', async () => {
    ...
    try{
      ...
      expect(e.response.message[1].detail).toBe("Must have required property 'type'")
      expect(e.response.message[2].detail).toBe("Must have required property 'id'")
      expect(e.response.message[3].detail).toBe('Must match exactly one schema: "null" or "object"')
      expect(e.response.message[4].detail).toBe("Must be array")
      expect(e.response.message[5].detail).toBe('Must match exactly one schema: "object" or "array"')
  ...
  it('Should be error, incorrect id is no number string', async () => {
    ...
    try{
      ...
      expect(e.response.message[2].detail).toBe('Must match exactly one schema: "null" or "object"')
      expect(e.response.message[3].detail).toBe("Must be array")
      expect(e.response.message[4].detail).toBe('Must match exactly one schema: "object" or "array"')
      expect(e.response.message.length).toBeGreaterThan(0)
    }
  })
  ...
  it('Should be error, array with id no number', async () => {
    const data = {
      data: [{type: 'type', id: 'wsdf'}, {type: 'type', id: '2'}]
    }
    expect.assertions(9);
    try{
      await pipe.transform(data as any)
      ...
      expect(e.response.message[5].detail).toBe('Must match exactly one schema: "null" or "object"')
      expect(e.response.message[6].detail).toBe('Must match exactly one schema: "object" or "array"')
      expect(e.response.message.length).toBeGreaterThan(0)
    }
  })
  ...
  it('Should be error, array with incorrect items', async () => {
    const data = {
      data: [{someProps: 'type'}, {someProps: 'type', id: 'wsdf'}, {type: 'type', id: '2'}]
    }
    ...
    try{
      ...
      expect(e.response.message[2].detail).toBe('Must match exactly one schema: "null" or "object"')
      expect(e.response.message[3].detail).toBe('Must be null')
      expect(e.response.message[4].detail).toBe("Must have required property 'type'")
      expect(e.response.message[5].detail).toBe("Must have required property 'id'")
      expect(e.response.message[6].detail).toBe('Must match exactly one schema: "null" or "object"')
      expect(e.response.message[7].detail).toBe('Must be null')
      expect(e.response.message[8].detail).toBe("Must have required property 'type'")
      expect(e.response.message[9].detail).toBe('Must match pattern "^\\d+$"')
      expect(e.response.message[10].detail).toBe('Must match exactly one schema: "null" or "object"')
      expect(e.response.message[11].detail).toBe('Must match exactly one schema: "object" or "array"')
      expect(e.response.message.length).toBeGreaterThan(0)
    }
  })
