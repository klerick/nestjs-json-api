...
export class BodyRelationshipPatchPipe<Entity> implements PipeTransform{
  ...
  constructor(
    ...
    this.validateFunction = this.ajvCall.getSchema<ResourceRequestObject<Entity>>('body-relationship-patch-schema');
  }
        ...
        switch (error.keyword){
          ...
          case 'oneOf':
            if (error.schemaPath === '#/$defs/dataSchema/oneOf') {
              errorMsg[errorMsg.length - 1] = 'Must match exactly one schema: "null" or "object"'
            } else {
