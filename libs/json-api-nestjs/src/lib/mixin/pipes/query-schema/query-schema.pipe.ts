...
import {Entity, PipeMixin, QuerySchemaTypes, ValidationError} from '../../../types';
...
export class QuerySchemaPipe implements PipeTransform {
  ...
  constructor(
    ...
    this.validateFunction = this.ajvCall.getSchema(`inputQuerySchema-${schemaName}`);
  }

  private getErrorMsg(field: string | keyof QuerySchemaTypes): string {
    switch (field) {
      case 'fields': {
        const fieldArray = Object.keys(this.validateFunction.schema['properties']['fields']['properties']);
        return `Incorrect props in fields object of query. For resource "${getEntityName(this.repository.target)}" allow "fields" of query params "${fieldArray.join(',')}"`;
      }
      case 'filter': {
