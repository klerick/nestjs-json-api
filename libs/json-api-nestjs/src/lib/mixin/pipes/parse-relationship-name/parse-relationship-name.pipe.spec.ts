...
describe('ParseRelationshipNamePipe', () => {
  ...
  const parseRelationshipNameMixinPip = parseRelationshipNameMixin(Users, mockConnectionName);
  ...
  beforeAll(async () => {
    ...
    pipe = module.get<ParseRelationshipNamePipe<Users>>(parseRelationshipNameMixinPip);
  });
  ...
  it('Should be error', async () => {
    try {
      ...
      expect(e.response.message[0].detail).toBe("Relation 'rereerer' does not exist in resource 'users'")
