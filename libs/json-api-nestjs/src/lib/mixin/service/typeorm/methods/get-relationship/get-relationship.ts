...
export async function getRelationship<T>(
  ...
  const primaryColumns = this.repository.metadata.primaryColumns[0].propertyName;
  const relPrimaryColumns = this.repository.metadata.relations.filter(i => i.propertyName === relName)[0].inverseEntityMetadata.primaryColumns[0].propertyName;
  ...
  return {
    ...
    data: this.transform.transformData(result, [relName])['relationships'][relName]['data']
  }
}
