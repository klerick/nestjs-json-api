...
import {Addresses, mockDBTestModule, Users, Roles, Comments} from '../../../../../mock-utils';
...
describe('PostOne methode test', () => {
  ...
  beforeAll(async () => {
    ...
    await repository.manager.getRepository(Addresses).save(Object.assign(new Addresses(), {
      country: 'country',
      state: 'state'
    }))
  ...
  it('Should be error, array relation', async () => {
    ...
    try {
      ...
      expect(e.response.detail)
        .toBe(
          `Resource 'roles' with ids '${body.relationships.roles.data.map(i => i.id).join(',')}' does not exist`
        )
    }
  })
  ...
  it('Should be ok', async () => {
    ...
    expect(checkData.addresses.id).toEqual(parseInt(body.relationships.addresses.data.id, 10))
