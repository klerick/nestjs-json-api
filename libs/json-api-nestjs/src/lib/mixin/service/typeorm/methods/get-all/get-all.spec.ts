...
import {ConfigParam, FilterOperand, QueryField, QueryParams} from '../../../../../types';
...
describe('GetAll methode test', () => {
  ...
  it('check query with param', async () => {
    ...
    const createQueryBuilderSpy = jest.spyOn(repository, 'createQueryBuilder').mockImplementation((...alias) => {
    ...
    expect(resultJoinSpy).toHaveBeenNthCalledWith(1,`users.${include[0]}`, include[0]);
    expect(resultJoinSpy).toHaveBeenNthCalledWith(2,`users.${include[1]}`, include[1]);
  ...
  it('check query with param and select field', async () => {
    ...
    expect(resultJoinSpy).toHaveBeenNthCalledWith(1,`users.${include[0]}`, include[0]);
    expect(resultJoinSpy).toHaveBeenNthCalledWith(2,`users.${include[1]}`, include[1]);
    expect(resultJoinSpy).toHaveBeenNthCalledWith(3,`users.${include[2]}`, include[2]);
    expect(resultJoinSpy).toHaveBeenNthCalledWith(4,`users.${include[3]}`, include[3]);
