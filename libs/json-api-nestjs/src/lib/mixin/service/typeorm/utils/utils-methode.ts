import {EntityMetadata, Equal, In, Repository, SelectQueryBuilder} from 'typeorm';
import {NotFoundException, UnprocessableEntityException} from '@nestjs/common';
import {ObjectLiteral} from 'typeorm/common/ObjectLiteral';
import {FindOneOptions} from 'typeorm/find-options/FindOneOptions';

import {Filter, FilterOperand, ServiceOptions, ValidationError} from '../../../../types';
import {snakeToCamel, getEntityName, camelToKebab} from '../../../../helper';
import {OperandMapForNull, OperandMapForNullRelation, OperandsMap} from '../../../../types-common';
...
export class UtilsMethode {
  ...
  public static applyQueryFiltersTarget<T>(
    ...
    if (!UtilsMethode.relationNameMapByEntity.has(preparedResourceName)) {
      UtilsMethode.relationNameMapByEntity.set(preparedResourceName, new Set(metadata.relations.map(item => item.propertyName)));
    }

    const relations = UtilsMethode.relationNameMapByEntity.get(preparedResourceName);
    ...
    for (field in filter) {
      ...
      if (relations.has(field)) {
        const relation = metadata.relations.find(item => item.propertyName === field);
        const {
        ...
        switch (relation.relationType) {
          case 'many-to-many': {
            ...
            const relationProps = relation.isManyToManyOwner ? relation : relation.inverseRelation;
            const {joinTableName} = relationProps;
            const {databaseName: queryJoinPropsName} = relation.isManyToManyOwner ? inverseJoinColumns[0] : joinColumns[0];
            const {databaseName: selectJoinPropsName} = relation.isManyToManyOwner ? joinColumns[0] : inverseJoinColumns[0]
            ...
            resultExpression.push({
              expression: OperandMapForNullRelation[operand].replace('EXPRESSION', query),
              params: null
            });
            break;
          }
          case 'one-to-many': {
            const query = builder.subQuery()
              ...
              .where(`${resourceRelationName}.${inverseSidePropertyPath} = ${metadata.tableName}.id`)
              .getQuery();
            resultExpression.push({
              expression: OperandMapForNullRelation[operand].replace('EXPRESSION', query),
              params: null
          ...
          default: {
            resultExpression.push({
              expression: `${preparedResourceName}.${field.toString()} ${OperandMapForNull[operand].replace('EXPRESSION', UtilsMethode.getParamName(`${preparedResourceName}.${field}`, i))}`,
              params: null
            });
          }
        }
        ...
        resultExpression.push({
          expression: `${preparedResourceName}.${field.toString()} ${OperandsMap[operand].replace('EXPRESSION', UtilsMethode.getParamName(`${preparedResourceName}.${field}`, i))}`,
          params: {
            name: UtilsMethode.getParamName(`${preparedResourceName}.${field}`, i),
            val: value
          }
        });
      }
  ...
  public static applyQueryFilterRelation<T>(
    ...
    for (relationProperty in filter) {
      ...
      for (relationFieldProperty in filter[relationProperty]) {
        const operand = Object.keys(filter[relationProperty][relationFieldProperty]).pop();
        const value = operand === FilterOperand.like
          ? `%${filter[relationProperty][relationFieldProperty][operand]}%`
          : filter[relationProperty][relationFieldProperty][operand];

        const currentOperandMap = value.toString().toLocaleLowerCase() === 'null' ? OperandMapForNull : OperandsMap
        const paramsField = value.toString().toLocaleLowerCase() === 'null' ? null : UtilsMethode.getParamName(`${relationProperty}.${relationFieldProperty.toString()}`, i);
        switch (relation.relationType) {
          case 'many-to-many': {
            ...
            const query = builder.subQuery()
              ...
              .where(`${relationProperty}.${relationFieldProperty.toString()} ${currentOperandMap[operand].replace('EXPRESSION', paramsField)}`)
              .getQuery();
            resultExpression.push({
              expression: `${preparedResourceName}.id IN ${query}`,
              params: paramsField === null ? null : {
                val: value,
                name: paramsField
              }
            });
          ...
          case 'one-to-many': {
            const query = builder.subQuery()
              ...
              .where(`${resourceName}.${relationFieldProperty.toString()} ${currentOperandMap[operand].replace('EXPRESSION', paramsField)}`)
              .getQuery();
          ...
          default:
            resultExpression.push({
              expression: `${relationProperty}.${relationFieldProperty.toString()} ${currentOperandMap[operand].replace('EXPRESSION', paramsField)}`,
              params: paramsField === null ? null : {
                val: value,
                name: paramsField
              },
  ...
  public static async* asyncIterateFindRelationships<T>(
    ...
    if (!UtilsMethode.relationNameMapByEntity.has(preparedResourceName)) {
      UtilsMethode.relationNameMapByEntity.set(preparedResourceName, new Set(repository.metadata.relations.map(item => item.propertyName)));
    }
    ...
    while (countRelationsKeys > i) {
      ...
      const result = await repository.manager.getRepository(relationsTypeName).find({
        select: {
          id: true
        },
        where: {
      ...
      if ((isArray && (result.length === 0 || result.length !== idsToAdd.length)) || (!isArray && result.length === 0)) {
        const detail = isArray
          ? `Resource '${relationsTypeName}' with ids '${idsToAdd.join(',')}' does not exist`
      ...
      yield {propsName: relationsName, type, rel: isArray ? result : result[0], id: isArray ? idsToAdd : idsToAdd[0]};
      i++;
    }
  }
  ...
  public static async validateRelationRequestData<T>(
    ...
    const relationItem = repository.metadata.relations.find(i => i.propertyName === relName);

    const typeName = camelToKebab(getEntityName(relationItem.inverseEntityMetadata.target));

    const isArray = Array.isArray(body);

    if (['one-to-many', 'many-to-many'].includes(relationItem.relationType) && !isArray) {
    ...
    if (["one-to-one", "many-to-one"].includes(relationItem.relationType) && isArray) {
      throw new UnprocessableEntityException([{
        source: {
          parameter: '/data'
        },
    ...
    if (!item) {
      throw new NotFoundException([{
        detail: `Not exist item "${id}" in resource "${camelToKebab(getEntityName(repository.target))}"`
      }]);
    }
    ...
    const relData = await repository.manager.getRepository(relationItem.inverseEntityMetadata.target).find({
      select: {
        [relationItem.inverseEntityMetadata.primaryColumns[0].propertyName]: true
      },
      where: {
        id: In(relIds)
      }
    ...
    if (relData.length !== prepareData.length) {
      const mapRelData = relData.reduce<Record<string, boolean>>((acum, item) => {
        acum[item.id] = true
        return acum;
      }, {});
      throw new NotFoundException(
