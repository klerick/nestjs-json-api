...
import { TupleOfEntityRelation } from '../../mixin/types';
import { InputValidateData, ValidateReturn } from '../../type-orm/service';

type RelationshipsResult<E extends ObjectLiteral> = {
  [K in EntityRelation<E>]: E[K] extends Collection<E[K]> ? E[K] : E[K] | null;
};

function isRelationField<E extends ObjectLiteral>(
...
Injectable();
export class MicroOrmUtilService<E extends ObjectLiteral> {
  ...
  queryBuilder<T extends ObjectLiteral = E>(
    entity: EntityClass<T>,
    alias: string
  ...
  getFilterExpressionForTarget<T extends ObjectLiteral = E>(
    query: Query<T>
  ): QBFilterQuery<T>[] {
  ...
  getConditionalForJoin<T extends ObjectLiteral = E>(
    query: Query<T>,
    key: string
  ): QBFilterQuery {
    ...
    if (!(key in filterRelation)) return {};
  ...
  getFilterExpressionForRelation<T extends ObjectLiteral = E>(
    query: Query<T>
  ): QBFilterQuery<T>[] {
    const result: QBFilterQuery<T>[] = [];
    const filterRelation = this.getFilterObject(query, 'relation');
  ...
  getPrimaryNameFor(rel: EntityRelation<E>) {
  ...
  private getFilterObject<T extends ObjectLiteral>(
    query: Query<T>,
  ...
  private async *asyncIterateFindRelationships(
    relationships: NonNullable<Relationships<E>>
