...
describe('TypeormUtilsService', () => {
  ...
  beforeAll(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        ...
        CurrentEntityManager(),
        CurrentEntityRepository(UserGroups),
        TypeormUtilsService,
      ],
    }).compile();
    ...
    const moduleUsers: TestingModule = await Test.createTestingModule({
      imports: [mockDBTestModule(db)],
      providers: [
        ...providerEntities(getDataSourceToken()),
        CurrentDataSourceProvider(DEFAULT_CONNECTION_NAME),
  ...
  describe('asyncIterateFindRelationships', () => {
    it('should be ok', async () => {
      ...
      const data: PostData<Users>['relationships'] = {
        notes: {
          data: [
            {
              type: 'notes',
  ...
  describe('getFilterExpressionForRelation', () => {
    it('expression for relation many-to-many', () => {
      ...
      if (Array.isArray(first.params)) {
        ...
        expect(firstParams?.val).toBe(
          query.filter?.relation?.roles?.createdAt?.eq
        );
        ...
        expect(secondParams?.val).toBe(
          query.filter?.relation?.roles?.createdAt?.ne
        );
        ...
        expect(thirdParams?.val).toBe(
          query.filter?.relation?.roles?.createdAt?.nin
        );
