...
import { PatchData, PostData, Query } from '../../helper/zod';
import { CURRENT_ENTITY_REPOSITORY } from '../../constants';
import { TupleOfEntityRelation } from '../../helper/orm';
    ...
    for (const entries of ObjectTyped.entries(relationships)) {
      ...
      if (
        ...
        const message = isArray
          ? `Resource '${relationsTypeName}' with ids '${data
              .map((i) => i.id)
